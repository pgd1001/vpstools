...
const required = ['vps_health', 'vps_preflight_runbook', 'vps_execute_runbook', 'vps_get_execution', 'vps_search_audit', 'vps_verify_audit'];
for (const name of required) {
  if (!names.includes(name)) throw new Error(`missing MCP tool: ${name}`);
}
if (process.env.VPS_MCP_SMOKE_LIVE === 'true') {
  for (const name of ['vps_health', 'vps_whoami', 'vps_automation_status']) {
