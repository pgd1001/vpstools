...
- **External storage work is currently an integration foundation.** The artefact package now contains an isolated S3-compatible store with client-side encryption, optional S3 server-side encryption, retries, and checksum verification. The API composition root does not select it yet, so production deployments must continue to use local artefacts until the configuration wiring, migration tooling, and end-to-end tests are complete.
