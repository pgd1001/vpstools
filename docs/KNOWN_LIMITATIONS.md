...
- **The extended tier is only partly runtime-enabled.** PostgreSQL metadata now starts through the versioned Goose migrations and a live schema contract check. S3-compatible artifact storage can be selected when its complete configuration validates, and JetStream can be selected as the database-authoritative notification bridge. External scheduling, NATS event publishing, row-level security, and an independent horizontally scaled queue remain fail-closed or incomplete. The self-contained SQLite, local artefact, database polling, and embedded scheduler tier remains the default.
...
- **S3 deployments need a separate backup plan.** The `artifact-migrate` helper now copies local encrypted artefacts to S3, preserves IDs, writes a durable object manifest, verifies checksums, and refuses conflicts by default. The current `backup` command and restore smoke path still do not export or restore S3 objects, lifecycle state, database metadata, or S3-backed encryption keys as a complete deployment backup.
- **PostgreSQL is available as an opt-in metadata backend.** The API applies the checked-in migrations, verifies the live catalog, and uses the shared SQL runtime for current handlers. A production rollout still needs row-level security, a measured migration rehearsal, concurrency testing, and target-host recovery evidence before PostgreSQL should be used for a broad deployment.
