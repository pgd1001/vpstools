...
The self-contained tier is the default because it is easy to install, back up, and understand. Larger teams can move to PostgreSQL, S3-compatible storage, and JetStream without changing the CLI, web console, runbooks, or AI tools.
...
3. Copy local artefacts to S3-compatible storage, retaining stable artefact IDs and verifying checksums.
...
The self-contained SQLite, local artefact, database polling, and embedded scheduler path is the supported default. PostgreSQL, S3, and JetStream are configuration targets and architectural extension points. The complete export, replication, dual-read, reconciliation, and cutover tooling remains planned work. Treat the sequence above as the migration design and operational runbook, not as a claim that every migration command is already shipped.
